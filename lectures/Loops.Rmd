...
In this example, the list is two filenames: _basilisk.dat_ and _unicorn.dat_. Each time the loop iterates, it will assign a file name to the variable filename and run the `head` command. The first time through the loop, `$filename` is _basilisk.dat_. The interpreter runs the command head on _basilisk.dat_, and the prints the first three lines of _basilisk.dat_. For the second iteration, `$filename` becomes `unicorn.dat`. This time, the shell runs head on `unicorn.dat` and prints the first three lines of `unicorn.dat`. Since the list was only two items, the shell exits the for loop.
...
In this case, since the shell expands `$filename` to be the name of a file, `echo $filename` just prints the name of the file. Note that we _can’t_ write this as:
```{bash, eval=FALSE, echo=-1, engine="sh"}
cd ../data/data-shell/molecules
for filename in *.dat
do
...
```
because then the first time through the loop, when `$filename` expanded to _basilisk.dat_, the shell would try to run _basilisk.dat_ as a program. 
...
```{bash, echo = FALSE}
cd ../data/data-shell/creatures
#rm ../data/data-shell/molecules/*lengths*
