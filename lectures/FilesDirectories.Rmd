...
Let’s re-create that file and then move up one directory to _~/OpenDataSci/data/data-shell_ using `cd ..`:
...
Instead try: `rmdir thesis`

> __Challenge: What went wrong in this case?__

To really get rid of thesis we must also delete the file _draft.txt_. We can do this with the recursive option for `rm`:

```{bash, echo=-1, eval=F, engine='bash'}
cd ../data/data-shell
rm -r -i thesis
```
...
```{bash, echo=-1, eval=T, engine='bash'}
cd ../data/data-shell
mkdir thesis
...
Let’s create that directory and file one more time. (Note that this time we’re running touch with the path `thesis/draft.txt`, rather than going into the thesis directory and running nano or notepadd++ on _draft.txt_ there.)
...
```{bash eval=FALSE, include=TRUE, engine='bash'}
ls -F
	analyzed/ raw/
ls -F analyzed
...
```
```{bash eval=T, echo=F, engine='bash'}
cd ../data/data-shell
rm quotes.txt
```
