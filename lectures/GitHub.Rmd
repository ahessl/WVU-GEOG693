...
RStudio allows us to connect directly with GitHub through R-Projects.  An ```R-Project``` is a directory that contains all your scripts, data, functions, and output.  The extension is ```.Rproj```. To start a new project that will link to our new Rivers repo, click on ```File-New Project``` and select ```Version Control``` with ```Git```.  Give your new R-Project a name (```RiversProj```) and place it in your root directory. 

Now connect Jane should her remote repo by pasting the URL from GitHub into the ```Repository URL```.
...
* All questions can be answered here: https://happygitwithr.com/ especially see [Chapter 13](https://happygitwithr.com/rstudio-see-git.html)
...
Click on ```Commit```.  Now you can add a commit message and observe any changes you have made since the last commit.  Since this is your first commit everything is new and there are not conflicts!
...
Once the Jane has sent an email collaboration request, Tom can accept the button in the email.  Tom will be redirected to GitHub notifications: https://github.com/notifications, where he can accept access to the ```Rivers``` repository.  Tom will need to hit the ```clone or download``` button to get the URL for the remote repo, in my case:
	https://github.com/ahessl/Rivers.git
...
Next, the partner can start making changes to files in the repository. Make sure that your working directory is the cloned repository, and then make a new text file in that directory called INRivers.txt populated with the names of rivers in Indiana:

$ cd ~/Desktop/ahessl-Rivers
$ cat INRivers.txt
Wabash River
Muscatatuck River
Fall Creek
...
add and commit this new file to the repository using the push and commit commands: 

$ git add INRivers.txt
$ git commit -m “Added names of some rivers in Indiana to your list of state rivers”

IMPORTANT NOTE: You have only committed these files to your local repository.  You still need to push them to the online master at GitHub: 
...
To https://github.com/awdye/Rivers.git
   f634674..a2bd910  master -> master

Check back in on the owner’s remote repository at GitHub - you should see a new file called INRivers.txt. 

Now, the owner of the repository can pull the partner’s changes from the remote repository to their own local make additional changes: 

$ git pull origin master

At this point, all three repositories, i.e. the owner’s GitHub repo, and both partner’s local repositories are now in sync. 


How to Resolve Merge Conflicts

What if you and your collaborator(s) modify the same file in different ways?  This creates a conflict. To see what happens when you have a file in conflict, you and your partner should both modify WVRivers.txt, then attempt to commit and push the revised files to the remote repo. 
