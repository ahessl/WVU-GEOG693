...
cats$coat
```
Note, that the data with text values is recognized as a 'factor' by R.

We can do other operations on the columns (remember these are vectors). Say we discovered that the scale weighs one Kg light:
...
Values I rarely use, but you might:
```{r}
typeof(1+1i)
```
