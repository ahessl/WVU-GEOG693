...
Consider the file `data-shell/data/animals.txt`. After these commands, select which contents would be in `animalsUpd.txt`:  
...
In the first case, the input is `notes.txt` but in the second example, it is the contents of `notes.txt` that is the input.  
...
`a`
`test`
`Control-D` #to tell shell you are done entering stuff for wc
...
Nelle has run her samples through the assay machines and created 17 files in the `north-pacific-gyre/2012-07-03` directory contained within `data-shell`. As a quick sanity check, starting from her home directory, Nelle lists here files:
...
Hint: Experiment with the files in data/elements  
