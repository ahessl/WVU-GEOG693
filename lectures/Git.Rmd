...
Let's get started by making a repo in our home directory called ```~/Rivers```.
...
```{bash git status rivers, echo=-1, eval=T}
cd ~/Rivers
git status
```
...
#### Tracking Changes: 
First, create a text file that you can work with. In your terminal, create a file called “WVRivers.txt” and populate the file with some names of rivers in West Virginia. Later, we will make and track changes to this text file. You can create a new file in the terminal using the cat command: 
```{bash}
touch ~/Rivers/WVRivers.txt
```
...
```{bash WVRivers status, echo=-1, eval=T}
cd ~/Rivers
git status
```
...
```{bash rm git, eval=FALSE}
rm -rf .git
```

Remove the ```/Rivers``` repo
```{bash rm rivers}
cd ~/Rivers
rm -rf .git
```
...
#### RStudio and Git  
Later in the semester, we will use the Git client in the RStudio IDE to work with git.  To make get this feature in RStudio, you must be working in an R-project with version control.
